    ...
    <Sheet open={isOpen} onOpenChange={(open) => dispatch(setCartOpen(open))}>
      <SheetContent className="w-full sm:max-w-lg">
        <SheetHeader>
          ...
          <SheetDescription>
            {cart?.items?.length ? `${cart.items.length} items in your cart` : "Your cart is empty"}
          </SheetDescription>
        </SheetHeader>
                      ...
                      <div className="relative w-16 h-16 bg-muted rounded-md overflow-hidden">
                        <Image
                          src={
                            ...
                            `/placeholder.svg?height=64&width=64&query=${encodeURIComponent(item.product.title)}`
                          }
