    ...
    <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.2 }} className="group">
      <Card className="h-full flex flex-col overflow-hidden border-0 shadow-sm hover:shadow-md transition-shadow">
        <div className="relative aspect-square overflow-hidden bg-muted">
          <Image
            src={product.imageUrl || `/placeholder.svg?height=300&width=300&query=${encodeURIComponent(product.title)}`}
            alt={product.title}
            fill
        ...
        <CardFooter className="p-4 pt-0">
          <div className="flex items-center justify-between w-full">
            ...
            <Button size="sm" onClick={handleAddToCart} disabled={!product.inStock || isLoading} className="shrink-0">
